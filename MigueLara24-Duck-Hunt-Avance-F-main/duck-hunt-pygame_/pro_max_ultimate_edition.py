...
def draw_hud(surface, score, ammo, ducks_hit, ducks_total, round_num, chapter, active_powerups, combo_display, high_score, secret_mode=False, required_score=0):
    ...
    # MODIFICADO: Texto de ayuda actualizado para mouse
    hint_text = font_tiny.render("MOUSE=Mover | CLIC/ESPACIO=Disparar | R=Recargar", True, (100, 100, 110))
...
def draw_controls_screen(surface, frame):
    ...
    # MODIFICADO: Controles actualizados para mouse
    controls = [
        ("MOUSE", "Mover la mira"),
        ("CLIC IZQ / ESPACIO", "Disparar"),
        ("R", "Recargar"),
        ("ESC", "Pausar / Menú"),
    ]
    ...
    y += 35
    combo_title = font_medium.render("COMBOS SECRETOS (Teclas)", True, (255, 180, 100))
...
# ============ FUNCIÓN PRINCIPAL COMPLETA CON MOUSE ============
def main():
    running = True
    game_state = 'intro'
    menu_selection = 0
    ...
    # Posición inicial del cursor en el centro
    ...
    while running:
        ...
        # Control del cursor con MOUSE - MODIFICADO
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1 and game_state == 'playing':
                # Procesar disparo con clic del mouse
                if ammo > 0 and not showing_unlock_animation:
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if game_state == 'intro':
                    ...
                    # Combo detection (sigue funcionando con teclas)
                    ...
                    # Shoot con ESPACIO (alternativa al clic del mouse)
        ...
        if game_state == 'intro':
            if showing_unlock_animation:
                ...
                if unlock_animation_frame > 180:
                    ...
                    unlock_animation_frame = 0
                    particles = []
                    game_state = 'intro'  # ESTA ES LA LÍNEA CRÍTICA PARA SALIR DEL BUCLE
            else:
